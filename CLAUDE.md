...
**JSON 設定の集約**: JSON は文字列結合できないため `dotfile_merged_json` を使う。`dotfiles/<base>` のベースと有効プラグインの `dotfiles/<name>` 断片を jq でマージして `$HOME/<name>` を生成する。ベースが先・プラグインが後にマージされる。オブジェクトは再帰的にマージし（同名キーはプラグイン側が上書き）、**配列は連結してから重複を除去する**（jq の `*` 演算子は配列を置換してしまうため、独自の `merge` 関数を使っている）。これによりプラグイン側から `permissions.allow` のような配列へ追記できる。実例: `roles/common/default.rb` の `.claude/settings.json`（ベースは `dotfiles/.claude/settings.base.json`）と `.claude/mcp.json`（ベースは `dotfiles/.claude/mcp.base.json`）。差分は各プラグインの `dotfiles/.claude/{settings,mcp}.json`。

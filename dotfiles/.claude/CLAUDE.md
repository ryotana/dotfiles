...
## ファイル読み込み
- 1Mコンテキストをフル活用し、ファイルを確認・編集する前は必ず全体をReadツールで読むこと
- head/tail/grep/rg でファイルの一部だけ推測して回答・編集しないこと
    - ただし、明らかに巨大なファイル（ログ等）は例外とする

## 検証・動作確認
- 動作確認・テスト・差分確認などの検証フェーズでは、コマンド出力を grep/rg/head/tail 等で絞らず全ての出力を読み込むこと
    - 出力を絞ると差分や異常を見落とすため、特に検証フェーズでは省略しない
    - 絞り込みは探索・調査の初期段階に留める
